...
router.post("/votecandidate", function (req, res, next) {
  ...
  if (candidateIndex == undefined) {
    ...
    var sql =
      "UPDATE voterElection SET voteCount = '1' where voterId = ? and electionId = ?";
    db.query(sql, [voterId, electionId], function (err, result, fields) {
      if (err) throw err;
      var sql2 =
        "SELECT voteCount from candidateElection where candidateId = ? and electionId = ?";
      db.query(sql2, [candidateId, electionId], function (err, data, fields) {
        if (err) throw err;
        console.log(data);
        var sql3 =
          "UPDATE candidateElection SET voteCount = ? where candidateId = ? and electionId = ?";
        db.query(
          sql3,
          [data[0].voteCount + 1, candidateId, electionId],
...
router.post("/verifyvoter", function (req, res, next) {
  var aadhar_voter = req.body.aadhaar;
  console.log(req.body);
  ...
  db.query(sql, aadhar_voter, function (err, data, fields) {
    ...
    if (result===true) {
      ...
      res.cookie("voterDetails", voterDetails, {
        maxAge: 900000,
        httpOnly: false,
      });
