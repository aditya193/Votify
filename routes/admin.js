...
router.get("/verifyvoter/:id/:id2", function (req, res, next) {
    ...
    if (req.session.isAdmin == true) {
        ...
        var sq =
            "SELECT * FROM voterElection WHERE voterId = ? and electionId = ?";
        db.query(sq, [voterId, electionId], function (err, data, fields) {
            ...
            console.log(flag);
            var sq1 =
                "UPDATE voterElection SET verified = ? WHERE voterId = ? and electionId = ?";
            db.query(
                sq1,
                [flag, voterId, electionId],
                ...
                db.query(sql3, function (err, allVoters, fields) {
                    ...
                    for (var i = 0; i < election.length; i++) {
                        ...
                        for (var j = 0; j < voterElection.length; j++) {
                            if (
                                election[i].electionId ==
                                voterElection[j].electionId
                            ) {
                                for (var k = 0; k < allVoters.length; k++) {
                                    if (
                                        voterElection[j].voterId ==
                                        allVoters[k].aadhar
                                    ) {
                                        var obj = {
                    ...
                    for (var i = 0; i < election.length; i++) {
                        ...
                        for (var j = 0; j < allVoters.length; j++) {
                            ...
                            for (var k = 0; k < voterElection.length; k++) {
                                if (
                                    allVoters[j].aadhar ==
                                        voterElection[k].voterId &&
                                    voterElection[k].electionId ==
                                        election[i].electionId
                                )
                                    flag = 1;
                            }
                ...
                db.query(sql3, function (err, allCandidates, fields) {
                    ...
                    for (var i = 0; i < election.length; i++) {
                        ...
                        for (var j = 0; j < candidateElection.length; j++) {
                            if (
                                election[i].electionId ==
                                candidateElection[j].electionId
                            ) {
                                for (var k = 0; k < allCandidates.length; k++) {
                                    if (
                                        candidateElection[j].candidateId ==
                                        allCandidates[k].candidateId
                                    ) {
                                        var obj = {
                                            candidateId:
                                                allCandidates[k].candidateId,
                                            firstName:
                                                allCandidates[k].firstname,
                                            lastName: allCandidates[k].lastname,
                                            gender: allCandidates[k].gender,
                                            contact: allCandidates[k].contact,
                                            hasVoted:
                                                candidateElection[j].voteCount,
                                        };
                                        tempList.push(obj);
                                    }
                                }
                    ...
                    for (var i = 0; i < election.length; i++) {
                        ...
                        for (var j = 0; j < allCandidates.length; j++) {
                            ...
                            for (var k = 0; k < candidateElection.length; k++) {
                                if (
                                    allCandidates[j].candidateId ==
                                        candidateElection[k].candidateId &&
                                    candidateElection[k].electionId ==
                                        election[i].electionId
                                )
                                    flag = 1;
                            }
...
router.post("/candidateelection", function (req, res, next) {
    ...
    var sql =
        "INSERT INTO candidateElection (electionId, candidateId) values ?";
...
router.post("/debarcandidate", function (req, res, next) {
    ...
    var sql =
        "DELETE from candidateElection where candidateId = ? and electionId = ?";
    db.query(
        sql,
        [object.candidateId, object.electionId],
...
router.post("/addvoters", function (req, res, next) {
    ...
    var sql =
        "INSERT INTO voterElection (electionId, voterId, voteCount) values ?";
    db.query(sql, [inputData], function (err, result, fields) {
        if (err) throw err;
        res.redirect("/admin/addvoters");
    });
...
router.post("/debarvoters", function (req, res, next) {
    ...
    db.query(
        sql,
        [object.voterId, object.electionId],
        function (err, result, fields) {
            if (err) throw err;
            res.redirect("/admin/addvoters");
        }
    ...
    if (req.session.isAdmin == true) {
        ...
        db.query(sq, function (err, data, fields) {
            ...
            db.query(sql1, candidId, function (err, data, fields) {
                if (err) throw err;
                var sql2 =
                    "DELETE FROM candidateelection WHERE candidateId = ?";
...
router.get("/authvoter", function (req, res, next) {
    if (req.session.isAdmin) {
        var sql =
            "SELECT voterElection.*, voter.* from voterElection LEFT JOIN voter ON voterElection.voterId = voter.aadhar";
        db.query(sql, function (err, data, fields) {
            if (err) throw err;
            // console.log(data);
